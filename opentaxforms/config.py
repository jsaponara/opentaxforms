...
def parseCmdline():
    ...
    parser = ArgumentParser(
        description='Automates tax forms and provides an API for new tax form interfaces; must specify either form or directory option'
        )
    parser.add_argument('-f', '--form', dest='rootForms', nargs='*',
    ...
    # functionality.  [f8903 is referenced at f1040/line35] *very doable: eg
    # for "--year 2015", either 1. attempt to download each url from f8903--
    # 2015 on back to f8903--2010 2. OR download entire listing of irs-prior/
    # and use latest 8903 year not-greater-than 2015 parser.add_argument('-y',
    # '--year', dest='formyear', nargs='?', default=defaults.latestTaxYear,
    # help='form year, used only if forms will be downloaded, defaults to
    # latestTaxYear; eg 2013')
    parser.add_argument('-d', '--directory', dest='dirName', default=defaults.
    ...
    parser.add_argument('-D', '--dontDownload', dest='okToDownload',
        help='report error for forms not present locally',
        action="store_false")
    parser.add_argument('-q', '--quiet', help='suppress stdout',
        action="store_true")
    parser.add_argument('-r', '--recurse',
        help='recurse thru all referenced forms', action="store_true")
    parser.add_argument('-R', '--recurselevel', type=int,
        help='number of levels to recurse thru, defaults to infinite',
        dest='maxrecurselevel', default=defaults.maxrecurselevel)
    parser.add_argument('-k', '--skip', nargs='?', default=[],
        help='steps to skip, can be any combination of: ' + ' '.join('='.join(
        (k, v)) for k, v in SkippableSteps.items()), dest='skip')
    parser.add_argument('-C', '--ignoreCaches',
    ...
    parser.add_argument('-V', '--version', help='report version and exit',
        default=False, action="store_true")
    parser.add_argument('-Z', '--dropall', help='drop all database tables',
        action="store_true")
    parser.add_argument('--calledFromCmdline',
    ...
    if not ut.exists(allpdfpath):
        if 1:  # until resolve urllib2 code below
            ...
            try:
                ...
                if fin.getcode() != 200:
                    raise Exception('getFileList/urlopen/getcode=[%d]' % (fin.
                        getcode(), ))
...
def setup(**overrideArgs):
    ...
    if dirName is not None:
        ...
        if rootForms:
            cfg.formsRequested = [Form(rootForm, RecursionRootLevel) for
                rootForm in rootForms]
        else:
            from os import listdir
            from os.path import isfile, join as joinpath
            cfg.formsRequested = [Form(f, RecursionRootLevel) for f in listdir
                (dirName) if isfile(joinpath(dirName, f)) and f.lower().
                endswith('.pdf')]
        if not cfg.formsRequested and not cfg.relaxRqmts:
            raise Exception(
                'must specify either a form via -f or a directory with form pdf files via -d'
                )
